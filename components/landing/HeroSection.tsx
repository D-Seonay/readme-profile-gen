...
export const HeroSection = () => {
  ...
  return (
    <section className="relative pt-32 pb-20 overflow-hidden flex flex-col items-center text-center px-6">
      ...
      <div className={`absolute top-0 left-1/2 -translate-x-1/2 w-[1000px] h-[600px] blur-[120px] rounded-full -z-10 transition-colors duration-500 ${isDark ? 'bg-indigo-500/10' : 'bg-indigo-500/5'}`} />
      ...
      >
        ...
        >
          {t.landing.hero.tagline.split(' ').map((word, i) => (
            <span key={i} className={word.toLowerCase().includes('github') ? 'text-indigo-500' : (isDark ? 'text-zinc-100' : 'text-zinc-900')}>
              {word}{' '}
            </span>
          ))}
        </motion.h1>
        ...
        <motion.p 
          className={`${isDark ? 'text-zinc-400' : 'text-zinc-500'} text-lg md:text-xl font-mono mb-10 max-w-2xl mx-auto`}
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          <Link 
            href="/builder"
            className={`group relative inline-flex items-center justify-center px-8 py-4 font-black italic uppercase tracking-widest overflow-hidden transition-all hover:pr-12 ${isDark ? 'bg-zinc-100 text-zinc-950' : 'bg-zinc-900 text-white'}`}
          >
