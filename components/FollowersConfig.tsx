    ...
    <div className={`space-y-6 pt-6 border-t border-zinc-800 transition-colors ${isDark ? 'text-zinc-100' : 'text-zinc-900'}`}>
      ...
      <div className="space-y-3">
        ...
        <div className={`flex p-1 rounded-xl border transition-colors ${isDark ? 'bg-zinc-900 border-zinc-800' : 'bg-zinc-100 border-zinc-200'}`}>
          <button
            onClick={() => setFollowersMode('badges')}
            className={`flex-1 px-3 py-2 text-[10px] font-mono uppercase rounded-lg transition-all ${
              followersMode === 'badges' ? (isDark ? 'bg-zinc-100 text-zinc-950 shadow-sm' : 'bg-white text-zinc-950 shadow-sm') : (isDark ? 'text-zinc-500 hover:text-zinc-300' : 'text-zinc-400 hover:text-zinc-600')
            }`}
          >
            {t.followers.badges}
          </button>
          <button
            onClick={() => setFollowersMode('list')}
            className={`flex-1 px-3 py-2 text-[10px] font-mono uppercase rounded-lg transition-all ${
              followersMode === 'list' ? (isDark ? 'bg-zinc-100 text-zinc-950 shadow-sm' : 'bg-white text-zinc-950 shadow-sm') : (isDark ? 'text-zinc-500 hover:text-zinc-300' : 'text-zinc-400 hover:text-zinc-600')
            }`}
          >
            {t.followers.list}
          </button>
